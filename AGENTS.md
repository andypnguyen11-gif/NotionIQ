...
- **Conventional commits _with a scope_:** `type(scope): subject` — e.g. `feat(charts): add filter engine`, `fix(auth): tighten route matcher`, `ci(github): add lint step`, `docs(spec): harden ADR-2`. Types: `feat fix chore docs ci test refactor perf`. Subject is lowercase, imperative, no trailing period.
...
- **All commits are authored by the repo owner** (`andypnguyen11-gif <andypnguyen11@gmail.com>`). **No AI / co-author / "Generated by" / "Co-Authored-By" attribution of any kind** — not in messages, subjects, or trailers.
- One branch per feature; **never build directly on `main`.**
- **Branch names describe the feature only** — no milestone, task, or PR identifiers (`notion-connect`, `chart-filters`, `embed-auth`), never `m1-notion-connect` or `task-3-foo`.
